...
class ShellTask extends BaseTask {
  ...
  async execute(colors) {
    ...
    const { filter, scope: scopeOption, includeAll, npmLogLevel:loglevel } = options
    ...
    for (const [index, project] of projects.entries()) {
      await this.executeCommand(colors, Object.assign({ scope, filters, npmLogLevel }, project), index, projects.length)
    }
  }

  async executeCommand(colors, project, index, total) {
    ...
    const result = await ShellWrapper.execute(command, verbose ? 'inherit' : 'ignore')
    const end = Date.now()
