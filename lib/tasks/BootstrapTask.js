...
class BootstrapTask extends ShellTask {
  ...
  installExternalDependencies({ otherDependencies, npmLogLevel, workingDirectory, cache, cacheDirectory, directory }) {
    ...
    if (cache) {
      ...
      for (const d of otherDependencies) {
        const [ scope, name ] = d.split('/')
        if (name && !scopes.has(scope)) {
          scopes.add(scope)
          commands.push(`mkdir -p "${directory}/node_modules/${scope}"`)
        }
        if (name) {
          commands.push(`cd "${directory}/node_modules/${scope}" && ln -s ../../${relativePath}/node_modules/${scope}/${name} ${name}`)
        } else {
          commands.push(`cd "${directory}/node_modules" && ln -s ../${relativePath}/node_modules/${scope} ${scope}`)
        }
      }
    } else {
