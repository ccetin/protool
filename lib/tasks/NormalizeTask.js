...
class NormalizeTask extends BaseTask {
  ...
  async execute(colors) {
    ...
    const info = { workingDirectory, scope, config, cacheDirectory, configFilename, scopedOnly, strictVersion, updateModule }
    await this.executeCommand(colors, dryRun, info)
    return 0
  }

  async executeCommand(colors, dryRun, { workingDirectory, scope, config, cacheDirectory,
