...
class Explorer {
  ...
  static async listProjects(directory, filters = [], target = 'package.json') {
    const files = await Util.resolveFiles(directory, target)
    const list = files
      .filter( (f) => {
  ...
  static async buildProjectGraph(directory, scope, includeAll, filter, target) {
    const graph = new Digraph()
    const mapping = {}
    const projects = await Explorer.listProjects(directory, filter, target)
  ...
  static async orderedProjects(directory, scope, includeAll, filter, target) {
    const { graph, mapping } = await Explorer.buildProjectGraph(directory, scope, includeAll, filter, target)
    ...
    const list = topologicalOrder
      ...
      .map( (key) => {
        ...
        const list = graph.adjacentList(key)
        const scopedDependencies = list.map( (packageName) => {
          const data = graph.getData(packageName)
          const { directory } = data
          return { packageName, directory }
        })
        const otherDependencies = Explorer.getDependencies(packageDefinition, list)
