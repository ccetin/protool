...
class Cli {
  ...
  static async run(options) {
    try {
      ...
      const aliases = Object.keys(bin || {}).filter( (b) => { return b !== name })
      const params = Object.assign({ name, aliases, version, configName: 'project.json', cacheDirectory: '~/.cache' }, options)
  ...
  static build(config) {
    const { name, aliases, main, options = [], commands = [], version } = config
